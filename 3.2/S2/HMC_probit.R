...
gradient_tau <- function(y_index, tau, X){
  ...
  #rt[K] <- exp(tau[K])*(sum(logacd(dnormcutoff_l[[K]], pnormcutoff_l[[K]], pnormcutoff_u[[K-1]]))-sum(logacd(dnormcutoff_u[[K]], 0, pnormcutoff_u[[K]])))
  ...
  #rt[1] <- sum(exp(dnormcutoff_l[[1]]-pnormcutoff_l[[1]]))-sum(logacd(dnormcutoff_u[[K]], 0, pnormcutoff_u[[K]]))
  for(i in 2:K){
    rt[1] <- rt[1] + sum(((dnormcutoff_l[[i]]-dnormcutoff_u[[i-1]])/(pnormcutoff_l[[i]]-pnormcutoff_u[[i-1]])))
    #rt[1] <- rt[1] + sum(logabcd(dnormcutoff_l[[i]], dnormcutoff_u[[i-1]], pnormcutoff_l[[i]], pnormcutoff_u[[i-1]]))
  }
  for(j in 2:(K-1)){
    rt[j] <- sum(((dnormcutoff_l[[j]])/(pnormcutoff_l[[j]]-pnormcutoff_u[[j-1]]))) - sum(dnormcutoff_u[[K]]/(1-pnormcutoff_u[[K]]))
    #rt[j] <- sum(logacd(dnormcutoff_l[[j]], pnormcutoff_l[[j]], pnormcutoff_u[[j-1]])) - sum(logacd(dnormcutoff_u[[K]], 0, pnormcutoff_u[[K]]))
    for(k in (j+1):K){
      rt[j] <- rt[j] +sum(((dnormcutoff_l[[k]]-dnormcutoff_u[[k-1]])/(pnormcutoff_l[[k]] - pnormcutoff_u[[k-1]])))
      #rt[j] <- rt[j] +sum(logabcd(dnormcutoff_l[[k]], dnormcutoff_u[[k-1]], pnormcutoff_l[[k]], pnormcutoff_u[[k-1]]))
    }
    rt[j] <- exp(tau[j])*rt[j]
  }
  #rt[1] <- sum(dnorm(cutoff_l[[1]])/pnorm(cutoff_l[[1]]))+sum(((dnorm(cutoff_l[[2]])-dnorm(cutoff_u[[1]]))/(pnorm(cutoff_l[[2]])-pnorm(cutoff_u[[1]]))))+sum(((dnorm(cutoff_l[[3]])-dnorm(cutoff_u[[2]]))/(pnorm(cutoff_l[[3]])-pnorm(cutoff_u[[2]]))))-sum(dnorm(cutoff_u[[3]])/(1-pnorm(cutoff_u[[3]])))
