...
gradient_tau1 <- function(y_index, tau, X){
  K <- length(tau)
  #rt <- - diag(1/rep(100, K))%*%tau
  #n <- table(y)
  cut <- cumsum(exp(c(-Inf, tau[-1])))+tau[1]
  ...
  rt <- sum(dnorm(cutoff_l[[1]])/pnorm(cutoff_l[[1]]))+sum(((dnorm(cutoff_l[[2]])-dnorm(cutoff_u[[1]]))/(pnorm(cutoff_l[[2]])-pnorm(cutoff_u[[1]]))))+sum(((dnorm(cutoff_l[[3]])-dnorm(cutoff_u[[2]]))/(pnorm(cutoff_l[[3]])-pnorm(cutoff_u[[2]]))))-sum(dnorm(cutoff_u[[3]])/(1-pnorm(cutoff_u[[3]])))
  #rt[2] <- exp(tau[2])*(sum(((dnorm(cutoff_l[[2]]))/(pnorm(cutoff_l[[2]])-pnorm(cutoff_u[[1]]))))+sum(((dnorm(cutoff_l[[3]])-dnorm(cutoff_u[[2]]))/(pnorm(cutoff_l[[3]])-pnorm(cutoff_u[[2]]))))-sum(dnorm(cutoff_u[[3]])/(1-pnorm(cutoff_u[[3]]))))
  #rt[3] <- exp(tau[3])*(sum(((dnorm(cutoff_l[[3]]))/(pnorm(cutoff_l[[3]])-pnorm(cutoff_u[[2]]))))-sum(dnorm(cutoff_u[[3]])/(1-pnorm(cutoff_u[[3]]))))
  return(rt)
}

gradient_tau2 <- function(y_index, tau, X){
  K <- length(tau)
  rt <- - diag(1/rep(100, K-1))%*%tau[-1]
  cut <- cumsum(exp(c(-Inf, tau[-1])))+tau[1]
  cutoff_u <- list()
  cutoff_l <- list()
  for(j in 1:K){
  ...
  #rt <- sum(dnorm(cutoff_l[[1]])/pnorm(cutoff_l[[1]]))+sum(((dnorm(cutoff_l[[2]])-dnorm(cutoff_u[[1]]))/(pnorm(cutoff_l[[2]])-pnorm(cutoff_u[[1]]))))+sum(((dnorm(cutoff_l[[3]])-dnorm(cutoff_u[[2]]))/(pnorm(cutoff_l[[3]])-pnorm(cutoff_u[[2]]))))-sum(dnorm(cutoff_u[[3]])/(1-pnorm(cutoff_u[[3]])))
  rt[1] <- exp(tau[2])*(sum(((dnorm(cutoff_l[[2]]))/(pnorm(cutoff_l[[2]])-pnorm(cutoff_u[[1]]))))+sum(((dnorm(cutoff_l[[3]])-dnorm(cutoff_u[[2]]))/(pnorm(cutoff_l[[3]])-pnorm(cutoff_u[[2]]))))-sum(dnorm(cutoff_u[[3]])/(1-pnorm(cutoff_u[[3]]))))
  rt[2] <- exp(tau[3])*(sum(((dnorm(cutoff_l[[3]]))/(pnorm(cutoff_l[[3]])-pnorm(cutoff_u[[2]]))))-sum(dnorm(cutoff_u[[3]])/(1-pnorm(cutoff_u[[3]]))))
  return(rt)
}
...
HMC_tau1 <- function(gradient, energyH, eps, L, current_tau, y, X){
  x <- current_tau
  p <- rnorm(1, 0, 1)
  current_p <- p
...
HMC_tau2 <- function(gradient, energyH, eps, L, current_tau, y, X){
  x <- current_tau
  p <- rnorm(length(x)-1, 0, 1)
  current_p <- p
